...
public class FileController {
    ...
    public static void loadFile() {
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_PATH))) {
            ...
            while ((line = reader.readLine()) != null) {
                TaskModel task = stringToTask(line);
                taskList.add(task);
            }
        } catch (IOException e) {
    ...
    private static TaskModel stringToTask(String line) {
        ...
        return new TaskModel(name, description, finalDate, priority, category, status);
    }

    private static String taskToString(TaskModel task, String dateFormatted) {
        return task.getName() + "|" +
                task.getDescription() + "|" +
                dateFormatted + "|" +
                task.getPriority() + "|" +
